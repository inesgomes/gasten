"""_summary_
    This script creates interpolations between two images using a trained VAE.
"""
import os
import argparse
import wandb
...
def parse_args():
    ...
    parser.add_argument("--train", dest="train", help="Train VAE",
                        type=argparse.BooleanOptionalAction, default=False)
...
if __name__ == "__main__":
    ...
    if args.train is False:
        ...
        vae = VariationalAutoencoder(
            config_run['latent_dims'], device).to(device)
    ...
    interpolate_list = interpolate(
        vae, x_1, x_2, config_run['n_interpolations'])

    # get classifier and visualize probabilities
    print(" > calculating probabilities ...")
    net, _, _, _ = construct_classifier_from_checkpoint(
        f"{config['train']['step-2']['classifier'][0]}", device=device)
    with torch.no_grad():
        net.eval()
